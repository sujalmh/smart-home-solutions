...
- [ ] Define v2 domain contracts for `Home -> Room -> Device -> DeviceLog` and map legacy `Server/Client/SwitchModule` fields.
- [ ] Add backend modules for `rooms`, `devices`, and `device_logs` in `backend/app/models`, `backend/app/schemas`, and `backend/app/api`.
- [ ] Add DB migrations (Alembic) for `rooms`, `devices`, `device_logs` with FK constraints, timestamps, soft-delete flags, and indexes.
- [ ] Run backfill migration to seed default rooms and map existing clients/switch modules into stable `devices` records.
...
- [ ] Add room-based APIs: `GET /homes/{home_id}/rooms`, `GET /rooms/{room_id}/devices`, `POST /rooms/{room_id}/devices/{device_id}/commands`.
...
- [ ] Update Flutter data layer (`repositories`, `providers`) to consume room-scoped APIs and centralize ID normalization.
- [ ] Update app layout to room-first navigation: room sidebar (desktop) / drawer (mobile) plus room-scoped device cards.
